...
def _classify_agent_error(error: str, return_code: Optional[int] = None) -> Optional[str]:
    # A non-zero exit means the agent process never got to run the task: a missing interpreter on a
    # musl image, an import failure, a crash at startup. That is not a model outcome, so it is
    # classified before the text checks, which only describe how a *running* agent ended.
    if return_code:
...
class AnySweAgent(SimpleResponsesAPIAgent):
    ...
    async def _run_agent_in_sandbox(self, params: AnySweInstanceConfig) -> NeMoGymResponse:
        ...
        agent_error_kind = _classify_agent_error(agent_error, result.return_code if result is not None else None)
